...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            ...
            closestGhost = self.distancer.getDistance(pacmanPosition, ghostPos)
            if closestGhost < distance:
                distance = closestGhost
                ghosts.append(ghostPos)

        for state in ghosts:
            for action in legal:
                nextState = Actions.getSuccessor(pacmanPosition, action)
                cost = self.distancer.getDistance(nextState, state)
                if cost < closestGhost:
                    closestGhost = cost
                    move = action
        return move
